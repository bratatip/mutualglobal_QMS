        ...
        <tbody class="bg-zinc-100 divide-y divide-gray-300">
            @foreach ($quotes as $quote)
                <tr class="border-t border-gray-200"
                    data-quote-id="{{ $quote->uuid }}">
                    <!-- <td class="text-gray-500 text-sm text-center px-2 py-3">{{ ($quotes->currentPage() - 1) * $quotes->perPage() + $loop->iteration }}</td> -->
                    <td class="text-gray-500 text-sm text-center px-2 py-3 text-green-500"
                        title="Green Is PL & Red Is Ed !">{{ $quote->quote_no }}</td>
                    ...
                    <td class="flex flex-wrap sm:justify-end p-2">
                        <div class="flex justify-center mt-2 align-items-center">
                            <span
                                class="inline-block px-6 py-1  md:mt-2 border border-solid rounded-2xl  text-xs text-[#0F628B] hover:bg-gray-100 ml-2 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white font-bold ">
                                @if ($quote->insurer_name && $quote->policy_number)
                                    Delivered
                                @elseif($quote->insurer_name)
                                    Policy Awaited
                                @else
                                    Initiated
                                @endif
